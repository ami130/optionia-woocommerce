<?php
/**
 * A slider: the customer drags rather than types.
 *
 * ## Why the value is shown beside the track
 *
 * A slider with no readout is a control a customer cannot answer precisely. The
 * browser shows nothing by default, so *"how many centimetres did I choose?"*
 * has no answer without this — and on a size or an engraving depth that is the
 * whole question being asked.
 *
 * The number updates from `frontend.js`; the server-rendered value is the
 * starting position, correct before any script runs.
 *
 * ⚠️ **`min` and `max` are required for this type** (the API refuses a slider
 * without them), because a browser silently defaults to 0–100 and a merchant who
 * forgot would ship a range they never chose.
 *
 * ## Why `type="range"` rather than a text input
 *
...
$optionia_step = isset( $optionia_validation['step'] ) && is_numeric( $optionia_validation['step'] )
	&& (float) $optionia_validation['step'] > 0
		...
		: ( empty( $optionia_validation['integer_only'] ) ? '' : '1' );

?>
<div class="optionia-option optionia-option--range optionia-option--cols-<?php echo esc_attr( (string) $optionia_display['columns'] ); ?><?php echo $optionia_display['collapsed'] ? ' optionia-option--collapsed' : ''; ?>" data-optionia="option" data-optionia-option="<?php echo esc_attr( $optionia_id ); ?>">
	...
	<input
		...
		data-optionia-range="1"
	/>

	<?php
	/*
	 * The readout. `aria-hidden` because the input already announces its own
	 * value to a screen reader — a second live region would say it twice.
	 */
	?>
	<output
		class="optionia-option__range-value"
		for="<?php echo esc_attr( $optionia_input_id ); ?>"
		data-optionia="range-value"
		aria-hidden="true"
	><?php echo esc_html( '' !== $optionia_default ? $optionia_default : $optionia_min ); ?></output>
</div>
