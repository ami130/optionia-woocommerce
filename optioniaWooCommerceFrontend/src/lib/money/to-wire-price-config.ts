 ...
 *
 * 🔴 **Copied from the backend's `serialization/price-config.ts` (M20.6 audit).**
 * The dashboard reads the **authoring** projection, which sends the **stored**
 * shape verbatim — `amountMinor`, `basisPoints`, `freeCharacters`, `minQuantity`
 * — while the evaluators read the **wire** shape the plugin receives. The worked
 * example connected the two without this conversion and reported *"could not be
 * priced"* for **every** correctly configured value.
 *
 * ⚠️ **The shared fixture could not catch it**, and that is worth recording:
 * the fixture is written in wire shape deliberately, because it is what both
 * evaluators actually receive. A fixture in either evaluator's input shape would
 * test the evaluator against itself. So it proves the evaluator and says nothing
 * about the *caller* handing it the wrong dialect — which is precisely the
 * mistake this module now prevents.
 *
 * ## Why this exists
 *
