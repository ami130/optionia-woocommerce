...
import { evaluateRules, type Answers, type EvaluableRule } from './rule-evaluator';

/**
 * The shared rule fixture, executed by the **dashboard's** copy of the
 * TypeScript evaluator (M20.6, ADR-083).
 *
 * 🔴 **This repository is the THIRD to run these cases**, after the plugin's
 * PHP suite and the backend's TypeScript one. ADR-083 records why a copy rather
 * than a shared package — and warns that a copy nothing executes is how an
 * unproven evaluator ships green. M17.9 is the precedent: a third evaluator
 * arrived in the browser with its fixture under `tests/js`, the gate searched
 * `tests/unit` alone, and deleting the suite left every gate passing.
 *
 ...
 * are free to differ in import paths and idiom, and these cases are what prove
 * they still answer the same.
 *
...
/*
 * ⚠️ **Resolved from the project root, not `__dirname`.** The backend's copy is
 * CommonJS and has one; this file is ESM under Vitest and does not. `cwd()` is
 * the package root for both `vitest run` and `npm test`.
 */
const fixture = JSON.parse(
  readFileSync(join(process.cwd(), 'test', 'fixtures', 'shared', 'rule-fixtures.json'), 'utf8'),
) as Fixture;
