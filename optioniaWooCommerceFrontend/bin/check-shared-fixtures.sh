...
# --- 3. The captured wire contract — NOT this repository's concern ------------
#
# 🔴 **Deliberately absent, not forgotten.** `assignment-wire.json` is a capture
# of the assignment handshake between the plugin and the cloud; the dashboard is
# not either end of it and holds no copy. `check-fixture-parity.sh` enforces the
# subset rule — every file this repository holds must match the backend's — so
# the absence is checked there rather than asserted here.
#
# ⚠️ **A section removed silently is how a gate decays.** This comment is the
# record that the numbering skips nothing: if the dashboard ever consumes the
# assignment wire, this is where its hash check belongs.
...
EXPECTED_RULES_SHA='43b03c70514900b57d028644847a163f403b12690987ffdc8d2fa0e654d3d055'
