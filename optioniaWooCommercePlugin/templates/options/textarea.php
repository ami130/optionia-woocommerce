<?php
/**
 * A multi-line text option: the customer types several lines.
 *
 * ## Why this is not simply `text_field.php` with a taller box
 *
 * A `<textarea>` has no `value` attribute — its content is the element's text,
 * and putting a default in an attribute would render the literal string. It also
 * needs `rows`, and its `maxlength` counts what the customer typed including the
 * line breaks the resolver keeps, so the counter beside it means the same thing
 * it does for a single line.
 *
 * ⚠️ **The default is echoed between the tags, not escaped into an attribute.**
 * `esc_textarea()` exists for exactly this and is what WordPress uses; a value
 * escaped with `esc_attr()` would show `&#10;` where a newline belongs.
 *
...
<div class="optionia-option optionia-option--textarea optionia-option--cols-<?php echo esc_attr( (string) $optionia_display['columns'] ); ?><?php echo $optionia_display['collapsed'] ? ' optionia-option--collapsed' : ''; ?>" data-optionia="option" data-optionia-option="<?php echo esc_attr( $optionia_id ); ?>"<?php echo '' !== $optionia_styles ? ' style="' . esc_attr( $optionia_styles ) . '"' : ''; ?>>
	...
	><?php echo esc_textarea( $optionia_default ); ?></textarea>
